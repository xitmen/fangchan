<?php
/**
 * ACE  Copyright (c) 2014 vezhi.net
...
private $ll_log=false;
	...
	public function start() {
	  ...
	  //added: 2014年11月11日 增加高级接口开关
      if(array_key_exists('api_setting', $_W['account'])){
            $tmppp=json_decode($_W['account']['api_setting'],true);
                //$xml=WeUtility::array2xml(array('ToUserName'=>$this->message['from'],'FromUserName'=>$this->message['to'],'CreateTime'=>TIMESTAMP,'MsgType'=>'text','Content'=>$_W['account']['api_setting']));
                //ob_clean();
                //echo $xml;
                //ob_end_flush();exit;
            if(is_array($tmppp) && array_key_exists('advanced', $tmppp) && $tmppp['advanced']=='1'){ 
                  //自定义API微信端处理 yam  2014年10月21日
                  $GLOBALS['ACE']=$tmppp;
                  require_once __DIR__.'/../../vzapi/Core/E.php';
                  ACE\API\E::check($this->message['content'],$this);
            }
            unset($tmppp);
      }
			...
			//多客服接口  2014年09月29日  yam
			if($this->params['rule']==-1 && isset($_W['account']['enable_wechat']) && $_W['account']['enable_wechat']==1){
				WeUtility::logging('info','transfer_customer_service');
				$xml=WeUtility::array2xml(array('ToUserName'=>$this->message['from'],'FromUserName'=>$this->message['to'],'CreateTime'=>TIMESTAMP,'MsgType'=>'transfer_customer_service'));
				ob_clean();
				echo $xml;
				ob_end_flush();
				WeUtility::logging('info',$xml);
			}
			else{
				WeUtility::logging('info','not transfer_customer_service');
				if($this->params['rule']=='-1' && isset($this->response['Content']) && $this->response['Content']==''){
				}
				else{
	...
	private function matcherEventSubscribe() {
		$params = array();
		if (isset($GLOBALS['_W']['account']['modules']['qrcode']) && strexists($this->message['eventkey'], 'qrscene')) {
	...
	private function matcherEventLocation() {
		 $FromUserName  = $this->message['from'];
		 $data['latitude']   = $this->message['latitude'];
		 $data['longitude']  = $this->message['longitude'];
		 $data['precision']   = $this->message['precision'];
 
		 
	     pdo_update('fans', $data, array('from_user' => $FromUserName));
	}
	...
	//log...
	private function LL($a,$b=''){
		if($this->ll_log){
			if(function_exists('L')){
				L('engine--',$b.'-->'.(is_array($a)?json_encode($a,JSON_UNESCAPED_UNICODE):$a),true);
			}
		}
	}
}
...
class WeUtility {
	...
	public static function logging($level = 'info', $message = '') {
		...
		$filename = IA_ROOT . '/data/logs/' . date('Ymd') . '.txt';
