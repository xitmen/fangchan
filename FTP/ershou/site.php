...
class ErshouModuleSite extends ComVerControl {
	...
	{	
		...
		$items = pdo_fetchall("SELECT * FROM ".tablename('ace_ershou_house')." ".$condition." order by torder asc,id desc");
	...
	public function doMobileInfo() 
	{
		...
		if($id)
		{
			...
			else
			{
				...
				//判断是否关注过 
				$attention = pdo_fetch("SELECT * FROM ".tablename('ace_ershou_queue')." WHERE openid = :openid  and house_number = :house_number" , array(':house_number' => substr($item['number'], -8),':openid' => $_W['fans']['from_user']));
	...
	public function doMobileQueue() 
	{
		...
		if(!empty($_GPC['id']))
		{
			if($_W['fans']['from_user'])
			{
				...
				$house = pdo_fetch("SELECT * FROM ".tablename('ace_ershou_house')." WHERE id = :id" , array(':id' => $_GPC['id']));
				if(empty($house))
				{
					echo json_encode(
				...
				//判断是否关注过 
				$attention = pdo_fetch("SELECT * FROM ".tablename('ace_ershou_queue')." WHERE openid = :openid  and house_number = :house_number" , array(':house_number' => substr($house['number'], -8),':openid' => $_W['fans']['from_user']));
				if(empty($attention))
				{
					$member = pdo_fetch("SELECT mobile FROM ".tablename('ace_members')." WHERE from_user = :from_user" , array(':from_user' => $_W['fans']['from_user']));
					$detail = pdo_fetch("SELECT * FROM ".tablename('ace_ershou_house_details')." WHERE h_id = :id" , array(':id' => $_GPC['id']));
					$data = array();
					$data['nickname'] = '';//客户昵称
