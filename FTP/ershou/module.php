...
class ErshouModule extends WeModule {
	...
	**/
	public function doHouses ()
	{
		...
		{
			$id = intval($_GPC['id']);
			$row = pdo_fetch("SELECT id FROM ".tablename('ace_ershou_house')." WHERE id = :id", array(':id' => $id));
			if (empty($row)) {
		...
		elseif ($operation == 'paixu') {
			...
			foreach($ids as $k=>$id)
			{
			 pdo_update('ace_ershou_house', array('torder'=>$id), array('id' => $k));
			}
	...
	**/
	public function doQueue ()
	{
		...
		{
			$id = intval($_GPC['id']);
			$row = pdo_fetch("SELECT id FROM ".tablename('ace_ershou_queue')." WHERE id = :id", array(':id' => $id));
			if (empty($row)) {
				message('抱歉，信息不存在或是已经被删除！');
			}
