...
class IfansModuleSite extends WeModuleSite {
	...
	public function __construct(){
		...
		if($_W['account']['type'] == '1') {
			...
			$this->gateway['movefenzu'] = "https://api.weixin.qq.com/cgi-bin/groups/members/update?access_token=%s";
		}
	...
	//分组
		...
		public function doWebmovefenzu(){
		global $_W, $_GPC;
		$groups = iunserializer($_W['account']['groups']);
		if (checksubmit('submit')) {
			$data['openid'] = $_GPC['openid'];
			$data['to_groupid']=$_GPC['gid'];
			$dat = json_encode($data);
			$dat = urldecode($dat);
			$url = sprintf($this->gateway['movefenzu'], $this->token);
			$content = ihttp_post($url, $dat);
			$dat = $content['content'];
			$result = @json_decode($dat, true);
			if ($result['errcode'] == '0') {
				message('移动用户分组成功', referer(), 'success');
			}else{
				message("公众平台返回接口错误. <br />错误代码为：{$result['errcode']}<br />错误信息为: {$result['errmsg']} <br />错误描述为: " . account_weixin_code($result['errcode']));
			}
		}
		$openid = $_GPC['openid'];
		$list = pdo_fetch("SELECT * FROM ".tablename('fans')." WHERE `from_user` = '{$openid}'");
		include $this->template('movefenzu');
	}
	...
	public function doWebMedia1(){
		...
		$list = pdo_fetchall("SELECT * FROM ".tablename('twarticle')." WHERE weid = '{$_W['weid']}' $condition ORDER BY id DESC LIMIT ".($pindex - 1) * $psize.','.$psize);
		...
		$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('twarticle') . " WHERE weid = '{$_W['weid']}' $condition");
		$pager = pagination($total, $pindex, $psize);
	...
	public function doWebMedia(){
		...
		if ($foo == 'display') {
			...
			$list = pdo_fetchall("SELECT * FROM ".tablename('twarticle')." WHERE weid = '{$_W['weid']}' $condition ORDER BY id DESC LIMIT ".($pindex - 1) * $psize.','.$psize, $params);
			...
			if (!empty($id)) {
				$item = pdo_fetch("SELECT * FROM ".tablename('twarticle')." WHERE id = :id" , array(':id' => $id));
			...
			if (checksubmit('fileupload-delete')) {
				...
				pdo_update('twarticle', array('thumb' => ''), array('id' => $id));
			...
			if (checksubmit('submit')) {
				...
				if (empty($id)) {
					pdo_insert('twarticle', $data);
				} else {
					unset($data['createtime']);
					pdo_update('twarticle', $data, array('id' => $id));
				}
				message('文章更新成功！', $this->createWebUrl('media', array('foo' => 'display')), 'success');
			} else {
				include $this->template('media');
			}
		} elseif ($foo == 'delete') {
			$id = intval($_GPC['id']);
			$row = pdo_fetch("SELECT id, thumb FROM ".tablename('twarticle')." WHERE id = :id", array(':id' => $id));
			if (empty($row)) {
			...
			pdo_delete('twarticle', array('id' => $id));
			message('删除成功！', referer(), 'success');
		}
	...
	public function doMobileDetail() {
		global $_GPC, $_W;
		$id = intval($_GPC['id']);
		$sql = "SELECT * FROM " . tablename('twarticle') . " WHERE `id`=:id";
	...
	public function sendPics($from_user,$message){
		...
		$contents = pdo_fetchall("SELECT id,title,description,thumb FROM ".tablename('twarticle')." WHERE id IN ({$ids})");
		foreach ($contents as $key => $value) {
			$articles[] = array(
				...
				'url' => $_W['siteroot'].create_url('mobile/module/detail', array('name' => 'ifans', 'id' => $value['id'], 'weid' => $_W['weid'])),
				);				
			$i++;
			if ($i>8) {
				break;
